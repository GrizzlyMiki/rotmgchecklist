...
var hideGotState = 2; //  0 = all, 1 = to get, 2 = gotten
//  The first time the user clicks a filter the treatment is going to be different
var firstTimeClickFilter = true;
...
const abilityFilters = ['cloakFilter', 'poisonFilter', 'prismFilter', 'quiverFilter', 'trapFilter', 'starFilter', 'spellFilter', 'skullFilter', 'orbFilter', 'tomeFilter', 'scepterFilter', 'helmFilter', 'shieldFilter', 'sealFilter', 'wakizashiFilter'];
...
function fillItemGrid() {
    ...
    for (let [actualItem, attributesArray] of Object.entries(items)) {
        ...
        itemNameInWikiURL = wikiURL + actualItem.replace('. ', '-').replace('\'-', '-').replace('.', '-').replace(': ', '-').replace(' ', '-').replace('\'', '-');
        //  This is the template for the card of every item, we will store all content in a string
        gridContent += '<div class="col mb-4 mx-3 colitem' + attributes + '" id="' + actualItem + 'Card" style="min-width:15rem; max-width:17.5rem;"><div class="card" style="min-width:15rem; max-width:17.5rem;"><a href="' + itemNameInWikiURL + '" class="mt-3"><img src="resources/items/' + actualItem + '.png" style="display:block;margin:auto;height:40px;width:40px;" alt="Image" /></a><h2 class="itemName mb-0" style="text-align:center;">' + actualItem + '</h2><div class="card-body"><div class="row align-items-end"><div class="mx-auto mt-1" style="width:48px; height:48px;"><input class="img-fluid" type="image" src="resources/' + getBagType(actualItem) + '.png" style="outline:none;height:48px;width:48px;" oncontextmenu="return clickCounterSubstract(\'' + itemNameEscaped + '\')" onclick="clickCounter(\'' + itemNameEscaped + '\')" alt="Image"/></div><h1 class="mb-0 pb-0" onclick="clickCounterSubstract(\'' + itemNameEscaped + '\')" style="text-align:center;" id="'+ actualItem + '"></h1><div class="mx-auto pl-1" ><button data-toggle="modal" data-target="#exampleModalCenter" data-itemid="' + actualItem + '" type="button" id="button' + actualItem + '" class="btn btn-primary mt-2 mb-1 py-1 px-2"><i class="fas fa-camera"></i></button></div></div></div></div></div>';
    }
...
function clickFilter(filterID) {
    //  The treatment on first click is different for regular filters, in that case we deactivate every other filter too
    if(firstTimeClickFilter && filterID != 'whitebagFilter' && filterID != 'orangebagFilter') {
        firstTimeClickFilter = false;
        disableEveryFilter();
    ...
    if (document.getElementById(filterID).checked) {    //THE CHECKBOX IS BEING ACTIVATED
        //  We change the appearance of the button
        document.getElementById(filterID).parentElement.style.backgroundColor = '#6c757d';
        document.getElementById(filterID).parentElement.style.boxShadow = '0 0 0 0.1rem rgb(130, 138, 145,.5)';
        ...
        //  We change the appearance of the button
        document.getElementById(filterID).parentElement.style.backgroundColor = 'whitesmoke';
        document.getElementById(filterID).parentElement.style.boxShadow = '';
        //  These are special buttons/filters we use to activate or deactivate every other filter in it's group
        if(filterID == 'weaponFilter') {
            weaponFilters.forEach(element => {if(document.getElementById(element).checked) clickFilterNoApply(element);});
        } else if(filterID == 'abilityFilter') {
...
//  As the name says, disables every REGULAR filter
function disableEveryFilter() {
    weaponFilters.forEach(element => {document.getElementById(element).checked = false;});
    abilityFilters.forEach(element => {document.getElementById(element).checked = false;});
    armorFilters.forEach(element => {document.getElementById(element).checked = false;});
    document.getElementById('ringFilter').checked = false;
}

//  Applies all the filters in a fixed way, changing the order would mess up everything (probably)
function applyFilters() {
    ...
    weaponFilters.forEach(element => {if(document.getElementById(element).checked) changeDisplay(element.replace('Filter', ''), 'block')});
    abilityFilters.forEach(element => {if(document.getElementById(element).checked) changeDisplay(element.replace('Filter', ''), 'block')});
    armorFilters.forEach(element => {if(document.getElementById(element).checked) changeDisplay(element.replace('Filter', ''), 'block')});
    if(document.getElementById('ringFilter').checked) changeDisplay('ring', 'block');
...
function clickOnHideGot() {
    ...
    if (hideGotState == 0) {    //WE SHOW ALL SO THE BUTTON IS DEACTIVATED
        document.getElementById('gotFilter').parentElement.style.backgroundColor = 'whitesmoke';
        document.getElementById('gotFilter').parentElement.style.boxShadow = '';
    } else {
        document.getElementById('gotFilter').parentElement.style.backgroundColor = '#6c757d';
...
function hideGot() {
    ...
    for (let [actualItem, attributesArray] of Object.entries(items)) {
        ...
        if((hideGotState == 1 && contadorActual > 0)||(hideGotState == 2 && contadorActual < 1))
            document.getElementById(actualItem + 'Card').style.display ='none';
    }
}
